...
**Coding tools and how they differ.** Different coding tools are useful for different work: code edits, local checks, review tasks, and sandbox tests. Each one has a different kind of friction. One tool can be sharp but narrow. Another can be useful when the job needs local files, tools, schedules, and messaging in the same operating layer. another open coding tool makes you feel more of the raw edge. The honest answer is I use multiple tools, and when one is giving me a bad day I try the same problem on another and see if the rhythm comes back.
