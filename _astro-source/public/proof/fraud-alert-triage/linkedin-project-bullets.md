# Fraud Alert Triage Workflow — LinkedIn / project bullets

Public-safe draft copy for LinkedIn, resume project sections, recruiter messages, or application proof packets.

## One-line project description

Built a public-safe synthetic fraud-alert triage workflow that shows how suspicious financial signals can become reviewer-ready evidence packets while keeping final account, compliance, and escalation decisions human-owned.

## Short LinkedIn project blurb

I packaged a synthetic fraud-alert triage workflow as a public proof artifact for AI + finance + fraud operations. The page shows the workflow spine from suspicious signal to source evidence, rule explanation, reviewer-ready case note, and human approval boundary. It uses fictional data only and intentionally avoids production, compliance, or autonomous account-action claims.

## Resume / profile bullets

- Designed a public-safe fraud-alert triage proof workflow translating suspicious payment, device, geography, and recipient signals into a reviewer-ready evidence packet.
- Framed AI assistance around regulated-workflow boundaries: evidence collection, rule explanation, case-note drafting, triage priority, and human approval.
- Built a synthetic case-note example that supports fraud review without declaring fraud, filing regulatory reports, approving KYC, moving funds, or blocking accounts autonomously.
- Packaged the workflow as a recruiter-readable Bionic Banker proof page with explicit limitations, role relevance, and public-safety guardrails.

## Recruiter-facing summary

This artifact demonstrates applied AI product judgment in a regulated finance context: not just model output, but the operational layer around it — evidence, explanation, triage, documentation, and approval boundaries.

## Safe outreach sentence

I recently packaged a synthetic fraud-alert triage workflow showing how AI can support regulated financial review work by assembling evidence packets and draft case notes while keeping final decisions human-owned.

## Boundary language to keep

- Synthetic data only.
- Decision support only.
- Human approval required.
- No customer, employer, wallet, or client data.
- No compliance sign-off, SAR/KYC decision, or autonomous account/fund action claim.
